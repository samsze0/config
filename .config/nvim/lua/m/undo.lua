...
function M.undolist_entry_producer(opts, entries, alt_level)
  ...
  return coroutine.create(function()
    for i = #entries, 1, -1 do
      ...
      if #brief > 80 then brief = brief:sub(1, 80) .. "..." end
      ...
      if entries[i].alt ~= nil then
        ...
        repeat
          ...
          if opts.debug then coroutine.yield(alt_undolist_entry) end
...
M.get_undolist = function(opts)
  ...
  if not opts.coroutine then
    ...
    for i = 1, opts.max_entries do
      ...
      if not ok then error("Failed to produce undotree entry: " .. entry) end
    ...
    if opts.debug then vim.notify("Coroutine mode") end
    ...
    local function f(i)
      ...
      if coroutine.status(undolist_entry_producer) == "dead" then return end
      ...
      i = i + 1
      if i <= opts.max_entries then vim.schedule_wrap(f)(i + 1) end
    end
