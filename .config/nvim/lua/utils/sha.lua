-- Source:
-- https://github.com/jbyuki/instant.nvim/blob/master/lua/instant/sha1.lua

local bit = require("bit")

local function from8to32(b1, b2, b3, b4)
  return bit.lshift(b1, 24)
    + bit.lshift(b2, 16)
    + bit.lshift(b3, 8)
    + bit.lshift(b4, 0)
end

local function S(n, X) return bit.bor(bit.lshift(X, n), bit.rshift(X, 32 - n)) end
...
function M.sha1(bytes)
  ...
  for i = 64 - remain + 1, 64 do
    ...
    if 64 - i < 8 then byte = bit.bor(byte, len[64 - i + 1]) end
  ...
  for i = 1, #bytes, 64 do
    ...
    for t = 16, 79 do
      W[t] = S(
        1,
        bit.bxor(bit.bxor(W[t - 3], W[t - 9]), bit.bxor(W[t - 14], W[t - 16]))
      )
    end
