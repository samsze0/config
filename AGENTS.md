...
<!-- BEGIN harness-workers managed -->
## Optional OpenRouter workers

Use the using-workers skill when the user enables Pi/OpenRouter delegation for a task.
Keep the returned run_id with that task. Delegate scoped independent work with ownership,
acceptance criteria and validation commands. Own architecture, review, integration and final
verification in Codex. Review worker results before integrating; never assume a completed
worker is correct. A new task has no enabled run until the user requests workers.
<!-- END harness-workers managed -->
